...
const (
	...
	KogitoToolingGitRepositoryURI           = "https://github.com/jomarko/kogito-tooling"
	KogitoToolingGitRepositoryBranch        = "BAQE-1695"
)
...
func registerKieAssetLibrarySteps(ctx *godog.ScenarioContext, data *Data) {
	...
	ctx.Step("^Project kogito-tooling is cloned$", data.projectKogitoToolingIsCloned)
	...
	ctx.Step("^Project \"([^\"]*)\" assets are re-marshalled by standalone editors$", data.projectAssetsAreRemarshalledByStandaloneEditors)
...
func (data *Data) projectKogitoToolingIsCloned() error {
	framework.GetLogger(data.Namespace).Info("Cloning kogito-tooling project", "URI", KogitoToolingGitRepositoryURI, "branch", KogitoToolingGitRepositoryBranch, "clonedLocation", data.Location[KogitoTooling])

	cloneOptions := &git.CloneOptions{
		URL:          KogitoToolingGitRepositoryURI,
		SingleBranch: true,
	}

	var err error
	reference := KogitoToolingGitRepositoryBranch

	// Try cloning as branch reference
	cloneOptions.ReferenceName = plumbing.NewBranchReferenceName(reference)
	err = cloneRepository(data.Location[KogitoTooling], cloneOptions)
	// If branch clone was successful then return, otherwise try other cloning options
	if err == nil {
		return nil
	}
...
func (data *Data) projectAssetsAreRemarshalledByStandaloneEditors(project string) error {
	...
	installOutput, installErr := framework.CreateCommand("yarn").
		InDirectory(data.Location[KogitoTooling]).
		Execute()
	framework.GetLogger(data.Namespace).Debug(installOutput)

	if installErr != nil {
		framework.GetLogger(data.Namespace).Warn(installOutput)
		framework.GetLogger(data.Namespace).Warn(project + " 'yarn' failed due to: " + installErr.Error())
		return installErr
	}

	buildOutput, buildErr := framework.CreateCommand("yarn", "run", "build:dev").
		InDirectory(data.Location[KogitoTooling]).
		Execute()
	framework.GetLogger(data.Namespace).Debug(buildOutput)

	if buildErr != nil {
		framework.GetLogger(data.Namespace).Warn(buildOutput)
		framework.GetLogger(data.Namespace).Warn(project + " 'yarn run build:dev' failed due to: " + buildErr.Error())
		return buildErr
	}

	kieAssetPath := "kie_asset_path=" + data.Location[KieAssetLibrary] + "/kie-assets-library-generate/target/" + project + "/src/main/resources"

	files, err := ioutil.ReadDir(kieAssetPath)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, file := range files {
		...
		if strings.HasSuffix(file.Name(), ".dmn") || strings.HasSuffix(file.Name(), ".bpmn") {
			kieAsset := "cypress_kie_asset='" + file.Name() + "'"

			runOutput, runErr := framework.CreateCommand("yarn", "run", "test:it:external", kieAssetPath, kieAsset).
				InDirectory(data.Location[KogitoTooling] + "/packages/kie-editors-standalone").
				Execute()
			framework.GetLogger(data.Namespace).Debug(runOutput)

			if runErr != nil {
				framework.GetLogger(data.Namespace).Warn(runOutput)
				framework.GetLogger(data.Namespace).Warn(project + " 'yarn run test:it:external' failed due to: " + runErr.Error())
				return runErr
			}
		}
	}
